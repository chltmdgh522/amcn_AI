...
def extract_main_sentences(text, ratio=0.2):
    """
    주어진 한글 텍스트에서 주요 문장을 추출하는 함수.

    Parameters:
    - text (str): 요약할 텍스트.
    - ratio (float): 요약할 비율 (0에서 1 사이의 값).

    Returns:
    - main_sentences (str): 주요 문장들을 조합한 문자열.
    """
    # KoNLPy를 사용하여 문장 토큰화
    kkma = Kkma()
    sentences = kkma.sentences(text)

    # 문장들을 공백으로 연결하여 한 문장으로 만듦
    text = ' '.join(sentences)

    # Gensim을 사용하여 주요 문장 추출
    main_sentences = summarize(text, ratio=ratio, split=True)
...
# 주요 문장 추출
main_sentences = extract_main_sentences(text)
print("주요 문장들:")
